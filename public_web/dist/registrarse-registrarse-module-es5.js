...
var RegistrarsePage = /** @class */ (function () {
    function RegistrarsePage(router, dialogService, loginService, toastrService) {
        this.router = router;
    ...
    RegistrarsePage.prototype.open = function (dialog) {
        this.dialogService.open(dialog, { context: {
                title: 'Registro Ipsfa en linea',
            }, });
    };
    RegistrarsePage.prototype.add = function (position, status, dialog) {
        var _this = this;
        this.loading = true;
        ...
        if (this.clave != this.rclave) {
            status = 'danger';
            this.toastrService.show('Error en la clave deben ser identicas', "Ipsfa en l\u00EDnea", { position: position, status: status });
            this.clave = '';
            this.rclave = '';
            this.loading = false;
            this.isHidden = false;
        ...
        this.loginService.makeUser(this.iUsuario).subscribe(function (data) {
            status = 'success';
            //console.log(data);
            _this.loading = false;
            _this.isHidden = false;
            if (data.msj != undefined) {
                _this.toastrService.show(data.msj, "Ipsfa en l\u00EDnea", { position: position, status: status });
            }
            else {
                _this.toastrService.show('Ok, proceso exitoso', "Ipsfa en l\u00EDnea", { position: position, status: status });
                _this.open(dialog);
            }
            ...
            _this.toastrService.show(e, "Ipsfa en l\u00EDnea", { position: position, status: status });
        });
    };
    RegistrarsePage.prototype.continuar = function () {
        this.router.navigate(['login']);
    };
    RegistrarsePage.prototype.seleccionarTipo = function (valor) {
        switch (valor) {
            case "TIT":
                this.lblcedula = 'Cédula del titular';
                break;
...
//# sourceMappingURL=registrarse-registrarse-module-es5.js.map